...
PreparedStatement pst = null;
	...
	public UpdateRoom() throws SQLException {
		...
		contentPane.add(lblNewLabel);
                ...
                try{
                    conn c = new conn();
                    ResultSet rs = c.s.executeQuery("select * from customer1");
                    while(rs.next()){
                        c1.add(rs.getString("s1"));    
                    }
                }catch(Exception e){ }
		...
		b1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				conn c = new conn();
                                ResultSet rs1 = c.s.executeQuery("select * from customer1 where s1 = "+s1);
                                
                                while(rs1.next()){
                                    txt_Room.setText(rs1.getString("s6"));    
                                }
                            }catch(Exception ee){}
                            try{
                                conn c  = new conn();
                                ResultSet rs2 = c.s.executeQuery("select * from room where room = "+txt_Room.getText());
                                while(rs2.next()){
